...
{
  if ((argc !=2)&&(argc !=5)&&(argc != 10)&&(argc != 11))
    {
  ...
  if (argc == 2){
    ...
    std::string Cpn0str;
    std::string Cpn1str;
    cout << "The following arguments are optional; leave blank to use default value.\nSigma_CM [" << myInfo.get_sigmaCM() <<" GeV]: ";
    getline(std::cin, sigCMstr);
    cout << "E* [" << myInfo.get_Estar() << " GeV]: ";
    getline(std::cin, Estarstr);
    cout << "Sigma_E* [" << myInfo.get_sigmaE() << " GeV]: ";
    getline(std::cin, sigEstr);
    cout << "Cpp0 [" << myInfo.get_Cpp0() << " %]: ";
    getline(std::cin, Cpp0str);
    cout << "Cpn0 [" << myInfo.get_Cpn0() << " %]: ";
    getline(std::cin, Cpn0str);
    cout << "Cpn1 [" << myInfo.get_Cpn1() << " %]: ";
    getline(std::cin, Cpn1str);
    
    if(!(sigCMstr.empty())){
      myInfo.set_sigmaCM(atof (sigCMstr.c_str()));
    }
    if(!(Estarstr.empty())){
      myInfo.set_Estar(atof (Estarstr.c_str()));
    }
    if(!(sigEstr.empty())){
      myInfo.set_sigmaE(atof (sigEstr.c_str()));
    }
    if(!(Cpp0str.empty())){
      myInfo.set_Cpp0(atof (Cpp0str.c_str()));
    }
    if(!(Cpn0str.empty())){
      myInfo.set_Cpn0(atof (Cpn0str.c_str()));
    }
    if(!(Cpn1str.empty())){
      myInfo.set_Cpn1(atof (Cpn1str.c_str()));
    }
  } else {
    Ebeam = atof(argv[2]);
    nEvents = atoi(argv[4]);
      if(argc == 10 or argc == 11){
	myInfo.setCustomValues(atof(argv[5]),atof(argv[6]),atof(argv[7]),atof(argv[8]),atof(argv[9]));

      }
      if(argc == 11){
	myInfo.set_sigmaE(atof(argv[10]));
      }
  ...
  for (int event=0 ; event < nEvents ; event++)
	...
	cerr << "Working on event " << event << "\n";
      ...
      // Pick random E*
      double dE = 0;
      double sigE = myInfo.get_sigmaE();
      double Estar = myInfo.get_Estar();
      if(sigE>0){
	do
	  {
	    dE = myRand.Gaus(0.,sigE);
	  } while (!(abs(dE)<Estar));
      }
      ...
      pCM_Mag = vCM.Mag();
      TVector3 vAm2 = -vCM;
      double EAm2 = sqrt(vCM.Mag2() + sq(mAm2+dE));
